...
then
    ...
    kvm_eth0="-net nic,vlan=0,name=Aground,macaddr=$mac_eth0,model=virtio"
else
...
then
    ...
    kvm_eth1="-net nic,vlan=1,name=An1,macaddr=$mac_eth1,model=virtio"
else
...
if [ -d /tmp/vde.ctl ];
then
    kvm_eth2="-net nic,vlan=2,model=virtio,name=Aobiwan,macaddr=$mac_eth2 "
    kvm_eth2="$kvm_eth2 -net vde,vlan=2,sock=/tmp/vde.ctl";
fi
