                                    ...
                                    const u_char *dat, const int dao_len)
{
    ...
    if(watching) {
	dn = dag_network::find_or_make_by_instanceid(dao->rpl_instanceid, dagid,
						this->debug,
						watching);
    } else {
	dn = dag_network::find_by_instanceid(dao->rpl_instanceid, dagid);
    }
...
int dag_network::build_target_opt(struct in6_addr addr, int maskbits)
{
    ...
    // SV this needs to send the full prefix
    maskbits = 128;
