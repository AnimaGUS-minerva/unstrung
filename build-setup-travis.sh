...
mkdir -p ${BUILDTOP}/${HOST}
    ...
    if [ ! -d $BUILDTOP/mbedtls ]; then (cd ${BUILDTOP} && git clone -b mcr_add_otherName_3 https://github.com/mcr/mbedtls.git ); fi
...
LIBPCAP=${BUILDTOP}/host/libpcap-${PCAP_TAG}/libpcap.a
if [ ! -x $BUILDTOP/host/tcpdump-${TCPDUMP_TAG}/tcpdump ]
then
    if [ ! -d ${BUILDTOP}/libpcap ]; then
        (cd ${BUILDTOP} && git clone -b libpcap-${PCAP_TAG} https://github.com/the-tcpdump-group/libpcap.git && cd libpcap && ./autogen.sh )
    fi
    if [ ! -d ${BUILDTOP}/tcpdump ]; then
        (cd ${BUILDTOP} && git clone -b tcpdump-${TCPDUMP_TAG} https://github.com/the-tcpdump-group/tcpdump.git && cd tcpdump && ./autogen.sh )
    fi
    ...
    if $build32 && [ ! -d "${BUILDTOP}/i386/tcpdump-${LIBPCAP_TAG}/." ]; then
        (cd ${BUILDTOP} && mkdir -p i386/libpcap-${PCAP_TAG} && cd i386/libpcap-${PCAP_TAG} && CFLAGS=-m32 ../../libpcap/configure --prefix=$HOME/stuff --target=i686-pc-linux-gnu && make LDFLAGS=-m32 CFLAGS="-m32 -fPIC")
        (cd ${BUILDTOP} && ln -f -s i386/libpcap-${PCAP_TAG} libpcap && mkdir -p i386/tcpdump-${LIBPCAP_TAG} && cd i386/tcpdump-${LIBPCAP_TAG} && CFLAGS=-m32 ../../tcpdump/configure --prefix=$HOME/stuff --target=i686-pc-linux-gnu && make LDFLAGS=-m32 CFLAGS="-m32 -fPIC")
        (cd ${BUILDTOP}/i386 && ln -s -f tcpdump-${LIBPCAP_TAG} tcpdump )
    fi

    (cd $BUILDTOP/${HOST}/libpcap-${PCAP_TAG} && make install)
fi

LIBPCAP_HOST_DIR=${BUILDTOP}/${HOST}/libpcap-${PCAP_TAG}
LIBPCAP_HOST=${LIBPCAP_HOST_DIR}/libpcap.a
LIBPCAP_DIR=${BUILDTOP}/${ARCH}/libpcap-${PCAP_TAG}
LIBPCAP=${LIBPCAP_DIR}/libpcap.a

TCPDUMP_HOST_DIR=${BUILDTOP}/${HOST}/tcpdump-${LIBPCAP_TAG}
TCPDUMP_HOST=${TCPDUMP_HOST_DIR}/tcpdump
TCPDUMP_DIR=${BUILDTOP}/${ARCH}/tcpdump-${LIBPCAP_TAG}
