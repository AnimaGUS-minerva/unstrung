...
bool check_dag(unsigned char opt, instanceID_t instanceID)
{
    if(instanceID==0) {
	fprintf(stderr, "while processing '%c'; --instance must preceed DODAG parameters\n",
                opt);
	usage();
    }
    return true;
